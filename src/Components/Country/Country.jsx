    ...
    <>
      ...
      <div className={`country-page-container`}>
        <button className="back-btn" onClick={handleBackToHome}>
          <svg fill={theme === 'Dark' ? `#ffffff` : `#000000`} height="20px" width="20px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
          ...
          </svg>Back</button>
        {
          countries.filter((country) => {
            ...
            return (country.cca3 === countryCode) || (country.alpha3Code === countryCode)
          }).map((country) => {
            ...
            return (
              <>
                <div key={country.cca3 || country.alpha3Code} className={`country-page-subcontainer ${country.cca3 || country.alpha3Code}`}>
                  <img src={country.flags.png} alt={`country-page-image ${country.cca3 || country.alpha3Code}`} className={`country-page-image ${country.cca3 || country.alpha3Code}`} />

                  <div className="country-page-descriptions-container">
                    <h1 className="country-page-name">{country.name.common || country.name}</h1>

                    <div className="country-page-description-subcontainer">
                      <div className="country-desc first-column">
                        <p className={`country-page-description country-native-name`}><strong>Native Name</strong> : {`${(country.name.nativeName?.[keys[0]]?.common || country.name.nativeName?.[keys[0]]?.official) || (country.nativeName)}`}
                        </p>
                      ...
                      <div className="country-desc second-column">
                        ...
                        <p className={`country-page-description country-currencies`}><strong>Currencies</strong> :  
                        {/* {`${Object.keys(country.currencies || {})}`} */}
                          {isError ? country.currencies.map((currency, index) => (
                            <span key={index}> {currency.name}</span>
                          )) : ` ${Object.keys(country.currencies || {})}`}
                        </p>
                        <p className={`country-page-description country-languages`}><strong>Languages</strong> :  
                          {isError ?
